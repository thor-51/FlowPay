...
> **Important:** Only use these after you have actually run the project, observed the metrics, and can speak to the numbers from your own benchmarking. Fabricating benchmark results on a resume is a serious professional risk. The bullets below are templates — replace the placeholder numbers with real observations from your own machine.

---

- Designed and built a production-style **event-driven payment processing platform** in Java 21 + Spring Boot 3: REST API → Kafka → async consumer, processing **[X] transactions/day** in local benchmarks with p99 latency under **[Y] ms**.

- Implemented **Redis-backed idempotency** using atomic SETNX semantics to prevent duplicate transfers under concurrent client retries, with a Postgres UNIQUE constraint as a secondary backstop.

- Engineered **deadlock-safe pessimistic account locking** by always acquiring `SELECT FOR UPDATE` row locks in ascending UUID order regardless of transfer direction, eliminating the classic A→B / B→A deadlock pattern.

- Built an **exponential-backoff retry system** over Kafka: transient failures progress through RETRYING → DEAD_LETTERED with configurable max-attempts and backoff, triggering a scheduled re-publish sweep rather than blocking a Kafka partition.

- Configured **Prometheus + Grafana observability** with custom Micrometer counters (throughput, success rate, retry rate), a latency histogram (p50/p95/p99), and live per-status gauges computed from the database on every scrape.

- Wrote **40 tests** across 7 test classes using JUnit 5, Mockito, and Testcontainers: unit tests for all service-layer logic, a `@WebMvcTest` slice for the controller tier (with `GlobalExceptionHandler` verified at the HTTP level), and a `@DataJpaTest` backed by a real Postgres container validating constraint semantics.

---

*Built as a portfolio project demonstrating Spring Boot backend engineering, distributed systems concepts, and production observability practices.*
